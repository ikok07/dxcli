...
impl EncodeHandler {
    pub fn handle_method(method: &EncodeMethod) -> Result<String, CommandHandlerError> {
        match method {
    ...
    fn encode_base64(options: &Base64Options) -> Result<String, CommandHandlerError>{
    ...
    fn encode_url(options: &URLOptions) -> Result<String, CommandHandlerError>{
        Ok(urlencoding::encode(&options.text).to_string())
    }

    fn encode_hex(options: &HexOptions) -> Result<String, CommandHandlerError>{
